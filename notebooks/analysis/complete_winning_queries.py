"""
Football Analytics — Winning Profiles Analysis (Full)
======================================================
11 queries covering all analytical dimensions:
  - mart_winning_profiles  (match-level result data)
  - mart_league_standings  (team-season aggregates)
  - mart_player_performance (player-level finishing efficiency)

Dataset: 18,085 matches, 5 leagues, 10 seasons (2014–2023).

Run from the project root:
    python notebooks/analysis/winning_profiles_queries_full.py
"""
...
def run_df(sql: str, label: str):
    """Run a query and return a DataFrame (used for Q1-Q6 where we want df objects)."""
    import pandas as pd
    print(f"\n{'='*70}")
    print(f"QUERY: {label}")
    print("=" * 70)
...
# =============================================================================
# Q1 — Core winning profile
# How do all key metrics differ across win / draw / loss?
...
# Do home/away teams show the same winning pattern?
# =============================================================================
print("\n" + "=" * 70)
...
# =============================================================================
# Q3 — League variation
# Does the "winning formula" differ by league?
# =============================================================================
print("\n" + "=" * 70)
...
# =============================================================================
# Q4 — Temporal trend
# Have winning team characteristics changed over 10 seasons?
# =============================================================================
print("\n" + "=" * 70)
...
# =============================================================================
# Q5 — Metric discriminating power
# Which metric has the biggest % gap between wins and losses?
# =============================================================================
print("\n" + "=" * 70)
...
# Goals/xG ratio split by result.
# =============================================================================
print("\n" + "=" * 70)
print("Q6 — xG efficiency (goals per xG) by result")
print("=" * 70)
rows = run(f"""
SELECT
    match_result,
    ROUND(AVG(avg_goals_scored),                                    3) AS avg_goals,
    ROUND(AVG(avg_xg_for),                                          3) AS avg_xg,
...
ORDER BY CASE match_result WHEN 'win' THEN 1 WHEN 'draw' THEN 2 ELSE 3 END
""")
print(f"{'result':<8} {'goals':>7} {'xg':>7} {'g/xg':>7} {'matches':>9}")
print("-" * 45)
for r in rows:
    print(f"{r.match_result:<8} {r.avg_goals:>7} {r.avg_xg:>7} {r.goals_per_xg:>7} {r.total_matches:>9,}")


# =============================================================================
# Q7 — Team rankings: which teams best exemplify the winning formula?
# Uses mart_league_standings (team-season level).
# =============================================================================
print("\n" + "=" * 70)
...
# =============================================================================
# Q9 — Temporal evolution (league-wide, all teams)
# How have key metrics evolved across all 10 seasons?
# Uses mart_league_standings for league-wide averages.
# =============================================================================
print("\n" + "=" * 70)
print("Q9 — Temporal evolution: league-wide metrics by season")
print("=" * 70)
rows = run(f"""
SELECT
    season,
